(function () {
  ...
  const EXCHANGE_CHAINS = [
    {
      ...
      steps: ["5 Синих Т3 → 50 Белых", "100 Белых → 80 Пыли", "100 Пыли → 10 Синих Т4"],
      costInT3Blue: 1.25,
      useBlue: true, useGreen: false, useWhite: false,
    },
    {
      id: "green_white_dust_t4",
      label: "Зелёный Т3 → Белый → Пыль → Синий Т4",
      steps: ["25 Зелёных Т3 → 50 Белых", "100 Белых → 80 Пыли", "100 Пыли → 10 Синих Т4"],
      costInT3Green: 6.25,
      useBlue: false, useGreen: true, useWhite: false,
    },
    {
      id: "green_dust_t4",
      label: "Зелёный Т3 → Пыль → Синий Т4",
      steps: ["50 Зелёных Т3 → 80 Пыли", "100 Пыли → 10 Синих Т4"],
      costInT3Green: 6.25,
      useBlue: false, useGreen: true, useWhite: false,
    },
    {
      id: "white_dust_t4",
      label: "Белый → Пыль → Синий Т4",
      steps: ["100 Белых → 80 Пыли", "100 Пыли → 10 Синих Т4"],
      costInWhite: 12.5,
      useBlue: false, useGreen: false, useWhite: true,
    },
  ];
  ...
  function initExchangeState() {
            ...
            PROFESSIONS.forEach(p => {
              if (parsed.prices[p.id] && typeof parsed.prices[p.id] === "object") {
                exchangeState.prices[p.id] = {
                  ...
                  white: sanitizeNumber(parsed.prices[p.id].white, 0),
    ...
    PROFESSIONS.forEach(p => {
      if (!exchangeState.prices[p.id]) {
        exchangeState.prices[p.id] = { t3blue: 0, t3green: 0, white: 0, t4blue: 0 };
      }
    });
  }
  ...
  function renderExchangePanel() {
    ...
    inputsEl.innerHTML = `
      ...
      ${renderExchangeInput("white", "⬜ Белый", prices.white)}
  ...
  function renderExchangeChains(chainsEl) {
    ...
    const whitePer1 = (prices.white || 0) / 100;
    const t4bluePer1 = (prices.t4blue || 0) / 100;
    ...
    chainsEl.innerHTML = `<div class="exchange-chain-title">Цепочки обмена</div>` +
      EXCHANGE_CHAINS.map(chain => {
        ...
        if (chain.id === "blue_white_dust_t4") {
          ...
          inputDesc = `${chain.costInT3Blue} Синих Т3 → 1 Синий Т4`;
        } else if (chain.id === "green_white_dust_t4") {
          inputCost = chain.costInT3Green * t3greenPer1;
          inputDesc = `${chain.costInT3Green} Зелёных Т3 → 1 Синий Т4`;
        } else if (chain.id === "green_dust_t4") {
          inputCost = chain.costInT3Green * t3greenPer1;
          inputDesc = `${chain.costInT3Green} Зелёных Т3 → 1 Синий Т4`;
        } else if (chain.id === "white_dust_t4") {
          inputCost = chain.costInWhite * whitePer1;
          inputDesc = `${chain.costInWhite} Белых → 1 Синий Т4`;
        }

        const t4value = t4bluePer1;
        const profit = t4value - inputCost;
        const efficiency = inputCost > 0 ? (profit / inputCost) * 100 : null;
        const profitClass = profit >= 0 ? "value-positive" : "value-negative";
        const cardClass = profit >= 0 ? "is-profit" : "is-loss";
        const profitSign = profit >= 0 ? "+" : "";

        const inputCostFormatted = numberFormatter.format(Math.round(inputCost * 100)) + " зол./100 Т4";
        const t4valueFormatted = numberFormatter.format(Math.round(t4value * 100)) + " зол./100 Т4";
        const profitFormatted = profitSign + numberFormatter.format(Math.round(profit * 100)) + " зол. на 100 Т4";
        const efficiencyFormatted = efficiency !== null
          ? `${efficiency >= 0 ? "+" : ""}${preciseFormatter.format(efficiency)}%`
          : null;

        const hasData = (chain.useBlue ? prices.t3blue > 0 : true)
          && (chain.useGreen ? prices.t3green > 0 : true)
          && (chain.useWhite ? prices.white > 0 : true)
          && prices.t4blue > 0;
        ...
        return `
          <article class="exchange-chain-card ${hasData ? cardClass : ""}">
            ...
            ${hasData ? `
              ...
              <div class="exchange-chain-efficiency ${profitClass}">
                Эффективность: <strong>${efficiencyFormatted}</strong>
                <span style="color:var(--muted);font-weight:400"> · ${inputDesc}</span>
              </div>
              <div class="exchange-chain-sub">
                Стоимость входа: ${inputCostFormatted} · Ценность Т4: ${t4valueFormatted}
              </div>
